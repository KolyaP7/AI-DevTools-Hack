...
async def enhance_audio_for_transcription(
    ...
    with tracer.start_as_current_span("enhance_audio_for_transcription") as span:
        ...
        try:
            ...
            await ctx.info(f"🔊 Обрабатываю аудио: {input_audio}")
            await ctx.info(f"📤 Выходной файл: {output_audio}")
            ...
            await ctx.info(f"🎛️ Применяю фильтр: {audio_filter}")
            ...
            if os.path.exists(output_path):
                ...
                await ctx.info(f"📊 Размер выходного файла: {file_size} байт")
